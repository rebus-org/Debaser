...
namespace Debaser.Internals.Schema
{
    class SchemaManager
    {
        ...
        readonly ClassMapProperty _revisionPropertyOrNull;

        public SchemaManager(string connectionString, string tableName, string dataTypeName, string sprocName, IEnumerable<ClassMapProperty> properties, string schema = "dbo", string extraCriteria = null)
        {
            ...
            _revisionPropertyOrNull = propertyList.SingleOrDefault(p => p.IsRevision);
            _properties = propertyList.Except(new[] { _revisionPropertyOrNull }).ToList();
            _keyProperties = propertyList.Where(p => p.IsKey).ToList();
...
");
                    ...
                    if (createTable)
                    {
                        var primaryKeyColumnList = string.Join(", ", _keyProperties.Select(p => $"[{p.ColumnName}]"));

                        if (_revisionPropertyOrNull != null)
                        {
                            primaryKeyColumnList = primaryKeyColumnList + $", [{_revisionPropertyOrNull.ColumnName}]";
                        }
...
{GetColumnDefinitionSql(8)},
        PRIMARY KEY({primaryKeyColumnList})
    )
...
");
                    ...
                    if (createProcedure)
                    {
                        if (_revisionPropertyOrNull != null)
                        {
                            AppendOnly(connection, transaction, schemaId);
                        }
                        else
                        {
                            NormalUpsert(connection, transaction, schemaId);
                        }
                    }
        ...
        void AppendOnly(SqlConnection connection, SqlTransaction transaction, int schemaId)
        {
            ExecuteStatement(connection, transaction, $@"
...
-- This script was automagically generated by Debaser.
    ...
    WHEN MATCHED {_extraCriteria} THEN 
       INSERT VALUES (
{GetInsertSql(10, includeOtherRevisionColumn: true)}
       )

    WHEN NOT MATCHED THEN
      INSERT VALUES (
{GetInsertSql(10, includeRevisionColumn: true)}
      )
...
");
        ...
        void NormalUpsert(SqlConnection connection, SqlTransaction transaction, int schemaId)
        {
            ExecuteStatement(connection, transaction, $@"
...
";
        ...
        string GetInsertSql(int indentation, bool includeRevisionColumn = false, bool includeOtherRevisionColumn = false)
        {
            var columns = _properties.Select(p => $"[S].[{p.ColumnName}]").ToList();

            if (includeRevisionColumn)
            {
                columns.Add($"[S].[{_revisionPropertyOrNull.ColumnName}]");
            }

            if (includeOtherRevisionColumn)
            {
                columns.Add($"[T].[{_revisionPropertyOrNull.ColumnName}]+1");
            }

            return string.Join(", " + Environment.NewLine,
                columns.Indented(indentation));
        }

        string GetColumnDefinitionSql(int indentation)
        {
            var columns = _properties.Select(p => $"{p.GetColumnDefinition()}").ToList();

            if (_revisionPropertyOrNull != null)
            {
                columns.Add(_revisionPropertyOrNull.GetColumnDefinition());
            }
