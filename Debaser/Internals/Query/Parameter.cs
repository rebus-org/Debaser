...
class Parameter(string name, object value)
{
    ...
    SqlDbType GetSqlDbType()
    {
        ...
        if (TypeMap.KnownTypes.TryGetValue(type, out var sqlDbType)) return sqlDbType;
...
{string.Join(Environment.NewLine, TypeMap.KnownTypes.Select(kvp => $"* {kvp.Key} => {kvp.Value}"))}
